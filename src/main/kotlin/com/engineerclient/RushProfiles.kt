...
import com.engineerclient.waypoints.BrwPackFiles
import com.engineerclient.waypoints.BrwWaypoints
import com.engineerclient.waypoints.loadWaypoints
 ...
 * (initially blank) EC waypoint packs, plus the logic that keeps the Blood Rush Waypoints
...
object RushProfiles {
    ...
    suspend fun ensureAllPacks() {
        ...
        if (created > 0) EngineerClient.logger.info("[ec] created $created blank profile packs")
    }
    ...
    fun effectiveClass(): String? {
        EcConfig.data.classOverride?.let { return it.takeIf { c -> c in CLASSES } }
        ClassDetect.detected?.let { return friendlyName(it) }
        return EcConfig.data.lastKnownClass?.takeIf { it in CLASSES }
    }

    fun activePackName(): String? {
        val clazz = effectiveClass() ?: return null
        return packName(clazz, EcConfig.data.playersOnRush, EcConfig.data.dedicatedDoor)
    }
    ...
    fun applySelection(reason: String) {
        if (!EcConfig.data.enabled) return
        ...
        val desired = (listOf(pack) + EcConfig.data.customPacks).distinct()
