 ...
 * - packs live in `config/engineerclient/waypoints/` instead of `config/odin/dungeon-waypoints/`
 * - `createPack` is SILENT when the pack already exists (EC ensures 40 packs on
 *   every launch; upstream chats an error per duplicate)
 * - no legacy-config migration (EC has no legacy format)
...
object BrwPackFiles {
    ...
    val packsFolder = File(Minecraft.getInstance().gameDirectory, "config/engineerclient/waypoints").apply { mkdirs() }
    ...
    suspend fun loadPack(packName: String): MutableMap<String, MutableList<DungeonWaypoint>> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            EngineerClient.logger.error("[ec] failed to load pack '$packName'", it)
            emptyPack()
        }
    }
    ...
    suspend fun savePack(packName: String, waypoints: MutableMap<String, MutableList<DungeonWaypoint>>) =
        withContext(Dispatchers.IO) {
            ...
            }.onFailure { EngineerClient.logger.error("[ec] failed to save pack '$packName'", it) }
        }
    ...
    suspend fun createPack(packName: String): Boolean = withContext(Dispatchers.IO) {
        when {
            ...
            packFile(packName).exists() -> false
                ...
                EngineerClient.logger.error("[ec] failed to create pack '$packName'", it)
                false
            }
        }
    }
    ...
    suspend fun deletePack(packName: String): Boolean = withContext(Dispatchers.IO) {
        runCatching { packFile(packName).takeIf(File::exists)?.delete() == true }
            .getOrElse {
                EngineerClient.logger.error("[ec] failed to delete pack '$packName'", it)
                false
            }
    }
        ...
        packsFolder.listFiles { f -> f.extension == "json" }
            ?.mapNotNull { file ->
                runCatching {
                    ...
                    EngineerClient.logger.error("[ec] failed to read pack file ${file.name}", it)
                    null
                }
            } ?: emptyList()
    }
