...
| EC file | Upstream file(s) |
|---|---|
| `BrwWaypoints.kt` | `dungeonwaypoints/DungeonWaypoints.kt` |
| `BrwWaypointPacks.kt` | `dungeonwaypoints/DungeonWaypointPacks.kt` |
| `BrwWaypointEditor.kt` | `dungeonwaypoints/DungeonWaypointEditor.kt` + `DungeonWaypointHud.kt` |
| `BrwSecretWaypoints.kt` | `dungeonwaypoints/SecretWaypoints.kt` |
| `BrwPackFiles.kt` | `config/WaypointPackFileUtils.kt` + serializers from `config/DungeonWaypointConfig.kt` |
...
1. Packs live in `config/engineerclient/waypoints/`, module config in `config/odin/addons/engineerclient.json`.
2. Room waypoints live in `BrwWaypoints.roomWaypoints`, NOT Odin's `room.waypoints`
...
5. `devMessage`/`modMessage` calls became `EngineerClient.logger` / `EngineerClient.chat`.
