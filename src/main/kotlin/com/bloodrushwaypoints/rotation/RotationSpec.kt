    ...
    val graph: Graph by lazy {
        try {
            val text = RotationSpec::class.java.getResourceAsStream("/rotation/p3.json")
                ...
                ?: return@lazy Graph().also { logger.warn("[brw] rotation/p3.json missing from the jar") }
            gson.fromJson(text, Graph::class.java).also {
                logger.info("[brw] rotation spec v${it.version}: ${it.roles.size} roles, ${it.pots.size} pots")
            }
        } catch (t: Throwable) {
            logger.warn("[brw] failed to parse the rotation spec", t)
            Graph()
        }
    }
}
