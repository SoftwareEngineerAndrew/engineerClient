...
/**
 * BRW's own log: one file per game session under `logs/brw/`, meant to be sent as-is when a run
...
object BrwLog {
    ...
    fun open(gameDir: Path, header: List<String>) {
        close()
        try {
            val dir = gameDir.resolve("logs").resolve("brw")
            Files.createDirectories(dir)
            val file = dir.resolve("brw-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss")) + ".log")
    ...
    private fun prune(dir: Path) {
        try {
            Files.list(dir).use { s ->
                s.filter { it.fileName.toString().startsWith("brw-") && it.fileName.toString().endsWith(".log") }
