...
object PovPreviews : Module(
     ...
     * Custom = BRW's own time-based replay, [smoothingTicks] ticks in the past.
     */
    ...
    private val keepRoleHud by BooleanSetting("Keep Role HUD", true, desc = "Redraws BRW's own role HUD over the previews.").withDependency { keepHuds }
    ...
    init {
        ...
        // boxes, the names, BRW's own ring — is submitted after the previews and lands on top.
        on<ScreenEvent.Render>(EventPriority.HIGHEST + 100) {
            BrwMod.safely("pov gui") { PovCapture.onScreenExtract(guiGraphics) }
        }
