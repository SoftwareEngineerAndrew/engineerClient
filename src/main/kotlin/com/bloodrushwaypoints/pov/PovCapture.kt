...
import com.bloodrushwaypoints.mixin.CameraAccessor
import com.bloodrushwaypoints.mixin.GameRendererInvoker
import com.bloodrushwaypoints.mixin.MinecraftAccessor
 ...
 *        CustomGUIImpl -> Odin's leap boxes, BRW's ring   (submitted after us -> on top)
 * GameRenderer.render
...
object PovCapture {
    ...
    private fun redrawKeptHuds(gfx: GuiGraphicsExtractor) {
        ...
        val scale = BrwMod.mc.window.guiScale.toFloat()
        if (scale <= 0f) return
        gfx.pose().pushMatrix()
        ...
        try {
            for (hud in ModuleManager.hudSettingsCache) {
                ...
                BrwMod.safely("pov keep hud ${hud.name}") { hud.value.draw(gfx, false) }
            }
        } finally {
            gfx.pose().popMatrix()
        }
    ...
    fun beforeLevelRender(deltaTracker: DeltaTracker): Boolean {
        ...
        val main = BrwMod.mc.mainRenderTarget
    ...
    private fun runPasses(deltaTracker: DeltaTracker): Int {
        ...
        try {
            ...
            fail(t)
            BrwMod.safely("pov camera restore") { restoreMainCamera(deltaTracker) }
        } finally {
            capturing = false
        }
    ...
    private fun renderFeed(index: Int, deltaTracker: DeltaTracker) {
        ...
        val saved = Saved(
            ...
            eyeHeight = cameraAccess.`brw$getEyeHeight`(),
            eyeHeightOld = cameraAccess.`brw$getEyeHeightOld`(),
        ...
        try {
            ...
            cameraAccess.`brw$setEyeHeight`(target.eyeHeight)
            cameraAccess.`brw$setEyeHeightOld`(target.eyeHeight)
            (mc as MinecraftAccessor).`brw$setMainRenderTarget`(feed)
            ...
            invoker.`brw$extractWindow`()
            invoker.`brw$extractOptions`()
            invoker.`brw$extractCamera`(deltaTracker, worldPartialTicks, camera.getCameraEntityPartialTicks(deltaTracker))
            ...
            (mc as MinecraftAccessor).`brw$setMainRenderTarget`(saved.target)
            ...
            cameraAccess.`brw$setEyeHeight`(saved.eyeHeight)
            cameraAccess.`brw$setEyeHeightOld`(saved.eyeHeightOld)
    ...
    private fun ensureFeeds(width: Int, height: Int) {
        ...
        feedHeight = height
        for (index in 0 until FEEDS) {
            val feed = TextureTarget("brw:pov$index", width, height, true)
    ...
    private fun findByName(name: String): Player? =
        BrwMod.mc.level?.players()?.firstOrNull { it.name.string.equals(name, ignoreCase = true) }

    private fun fail(t: Throwable) {
        disabledForSession = true
        freeRequested = true
        BrwMod.logger.error("[brw] pov preview failed — disabled for this session", t)
        BrwLog.log("WARN", "pov preview failed: ${t.javaClass.simpleName}: ${t.message}")
        BrwMod.chat("§8[§6BRW§8]§c POV previews hit an error and are off for this session §7(see the log).")
    }
}
